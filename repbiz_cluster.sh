...
if [[ $nhost -ne $nupservice ]];then
	echo "Service is Down on $ndownservice instances"
	exit 1
	else
		if [[ $nhost -eq $ndownservice ]]; then
			echo "CRITICAL: Service is down ON all $nhost instances"
			exit 2;
		fi
			if [[ $nhost -eq $nupservice ]];then
			echo "OK: Service $sname UP on all servers"
			exit 0
			fi
fi
